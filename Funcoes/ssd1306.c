...
void ssd1306_dashed_rect(ssd1306_t *ssd, uint8_t top, uint8_t left, uint8_t width, uint8_t height, bool value, bool fill) {
  // Linha superior e inferior tracejadas
  for (uint8_t x = left; x < left + width; x += 2) {  
      ssd1306_pixel(ssd, x, top, value);
      ssd1306_pixel(ssd, x, top + height - 1, value);
  }
  
  // Linhas laterais tracejadas
  for (uint8_t y = top; y < top + height; y += 2) {  
      ssd1306_pixel(ssd, left, y, value);
      ssd1306_pixel(ssd, left + width - 1, y, value);
  }
  if (fill) {
...
void ssd1306_rounded_rect(ssd1306_t *ssd, uint8_t top, uint8_t left, uint8_t width, uint8_t height, bool value) {
    // Linhas horizontais sem os cantos
    ssd1306_hline(ssd, left + 1, left + width - 2, top, value);
    ssd1306_hline(ssd, left + 1, left + width - 2, top + height - 1, value);

    // Linhas verticais sem os cantos
    ssd1306_vline(ssd, left, top + 1, top + height - 2, value);
    ssd1306_vline(ssd, left + width - 1, top + 1, top + height - 2, value);

    // Desenhando os cantos arredondados
    ssd1306_pixel(ssd, left + 1, top + 1, value);
    ssd1306_pixel(ssd, left + width - 2, top + 1, value);
    ssd1306_pixel(ssd, left + 1, top + height - 2, value);
    ssd1306_pixel(ssd, left + width - 2, top + height - 2, value);
}
